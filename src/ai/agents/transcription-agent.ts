...
import { fetchYouTubeTranscriptTool } from "../tools/fetch-youtube-transcript-tool.ts";
...
export const transcriptionAgent = new Agent({
  ...
  instructions: `
    ...
    Instructions:
    Input Parsing: When a user requests a transcript, extract the Video ID. If the user provides a full URL, parse the URL to isolate the ID.
    Tool Execution: Execute the ${fetchYouTubeTranscriptTool.id} using strictly the extracted Video ID.
